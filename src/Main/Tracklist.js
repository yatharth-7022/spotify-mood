  ...
  return (
    <div className="flex flex-col px-8 pb-4 text-white">
      {selectedAlbum.tracks.items.map((track, index) => (
        // track is an object containing 1 song each
        <div
          key={index}
        ...
        >
          <div className="flex gap-6">
            <div
              className={`${hoveredTrack === index ? "text-white" : "text-[#a0a0a0]"} flex items-center`}
            >
            ...
            <div className="flex flex-col">
              <div>{track.name}</div>
              <div
                className={`${hoveredTrack === index ? "text-white" : "text-[#a0a0a0]"}`}
              >
